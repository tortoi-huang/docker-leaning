...
mkdir -p "$MNT2HOME/.config/opencode"
touch "$MNT2HOME/.config/opencode/config.json"
...
echo "$MNT_USER_HOME/.config/opencode:       ${c_home}/.config/opencode"
...
docker run -it --rm \
    ...
    -v "$MNT_USER_HOME/.config/opencode":"${c_home}/.config/opencode" \
    -v "$MNT_USER_HOME/.bash_aliases":"${c_home}/.bash_aliases" \
    --env-file .env \
    opencode-java25:0.0.1